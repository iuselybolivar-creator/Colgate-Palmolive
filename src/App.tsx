...
// import ExcelJS from 'exceljs/dist/exceljs.min.js'; // REMOVED TOP LEVEL
// import { jsPDF } from 'jspdf'; // REMOVED TOP LEVEL
// import autoTable from 'jspdf-autotable'; // REMOVED TOP LEVEL
...
const ALLOWED_ADMIN_EMAILS = ['maam01proyectos@gmail.com', 'iuselybolivar@gmail.com'];
...
function AppContent() {
  ...
  const exportToExcel = async (fields: string[]) => {
    setIsSyncing(true);
    try {
      // Dynamic import to avoid top-level polyfill issues
  ...
  const exportToPDF = async (fields: string[]) => {
    setIsSyncing(true);
    try {
      // Dynamic imports to avoid top-level polyfill issues
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser && !ALLOWED_ADMIN_EMAILS.includes(currentUser.email?.toLowerCase() || '')) {
        signOut(auth);
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(doc(db, 'settings', 'about'), (snapshot) => {
      if (snapshot.exists()) {
        ...
        if (auth.currentUser && ALLOWED_ADMIN_EMAILS.includes(auth.currentUser.email?.toLowerCase() || '')) {
          setDoc(doc(db, 'settings', 'about'), defaultAboutSettings).catch(err => {
            console.warn("Failed to initialize default about settings (probably permissions):", err);
          });
        }
      }
    }, (error) => {
